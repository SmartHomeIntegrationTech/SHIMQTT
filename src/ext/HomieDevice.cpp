#include "ext/HomieDevice.h"

#include "ext/HomieNode.h"
...
void HomieDevice::Init() {
  ...
  if (!vecNode.size()) {
    ...
    /*
                    lots of mqtt:homie300:srvrm-lightsense:dummy#dummy triggered
       in the log
  ...
  mqtt.onConnect(
      std::bind(&HomieDevice::onConnect, this, std::placeholders::_1));
  mqtt.onDisconnect(
      std::bind(&HomieDevice::onDisconnect, this, std::placeholders::_1));
  mqtt.onMessage(std::bind(&HomieDevice::onMqttMessage, this,
...
bool HomieDevice::IsConnected() { return mqtt.connected(); }
  ...
  if ((int)(millis() - ulLastLoopSecondCounterTimestamp) >= 1000) {
    ...
    if (bRapidUpdateRSSI && (ulSecondCounter_Uptime & 1)) {
      ...
      if (iWiFiRSSI != iWiFiRSSI_Current) {
        ...
        Publish(String(strTopic + "/$stats/signal").c_str(), 2, true,
                String(iWiFiRSSI).c_str());
      }
    }
  ...
  if (mqtt.connected()) {
    ...
    if ((int)(millis() - ulHomieStatsTimestamp) >= 30000) {
      ...
      if (bInitialPublishingDone) {
        bError |=
            0 ==
            Publish(String(strTopic + "/$state").c_str(), ipub_qos, true,
                    "ready");  // re-publish ready every time we update stats
      }

      bError |= 0 == Publish(String(strTopic + "/$stats/uptime").c_str(), 2,
                             true, String(ulSecondCounter_Uptime).c_str());
      bError |= 0 == Publish(String(strTopic + "/$stats/uptime-wifi").c_str(),
                             2, true, String(ulSecondCounter_WiFi).c_str());
      bError |= 0 == Publish(String(strTopic + "/$stats/uptime-mqtt").c_str(),
                             2, true, String(ulSecondCounter_MQTT).c_str());
      bError |= 0 == Publish(String(strTopic + "/$stats/signal").c_str(), 2,
                             true, String(WiFi.RSSI()).c_str());

      if (bError) {
        ulHomieStatsTimestamp =
            millis() - (30000 - GetErrorRetryFrequency());  // retry in a while
      } else {
        ulHomieStatsTimestamp = millis();
      }

      //			csprintf("Periodic publishing: %i, %i,
      //%i\n",pub_return[0],pub_return[1],pub_return[2]);
    }

    if (bDoPublishDefaults &&
        (int)(millis() - ulPublishDefaultsTimestamp) > 0) {
    ...
    if (!bConnecting) {
      // csprintf("millis()-ulLastReconnect=%i
      // interval=%i\n",millis()-ulLastReconnect,interval);

      if (!ulLastReconnect ||
          (millis() - ulLastReconnect) > GetReconnectInterval()) {
...
void HomieDevice::onDisconnect(AsyncMqttClientDisconnectReason reason) {
  ...
  if (bConnecting) {
    ...
    bConnecting = false;
    // csprintf("onDisconnect...   reason %i..
...
void HomieDevice::onMqttMessage(char* topic, char* payload,
...
void HomieDevice::DoInitialPublishing() {
  ...
  if (ulInitialPublishing != 0 &&
      (int)(millis() - ulInitialPublishing) < iInitialPublishingThrottle_ms) {
    return;
  }
  ...
  if (bDebug)
    csprintf("IPUB: %i        Node=%i  Prop=%i\n", iInitialPublishing,
             iInitialPublishing_Node, iInitialPublishing_Prop);
#endif

  if (iInitialPublishing == 0) {
    bool bError = false;
    bError |= 0 == Publish(String(strTopic + "/$state").c_str(), ipub_qos, true,
                           "init");
    bError |= 0 == Publish(String(strTopic + "/$homie").c_str(), ipub_qos, true,
                           "3.0.1");
    bError |= 0 == Publish(String(strTopic + "/$name").c_str(), ipub_qos, true,
  ...
  if (iInitialPublishing == 1) {
    bool bError = false;
    bError |= 0 == Publish(String(strTopic + "/$localip").c_str(), ipub_qos,
                           true, WiFi.localIP().toString().c_str());
    bError |= 0 == Publish(String(strTopic + "/$mac").c_str(), ipub_qos, true,
                           WiFi.macAddress().c_str());
    bError |= 0 == Publish(String(strTopic + "/$extensions").c_str(), ipub_qos,
                           true, "");
  ...
  if (iInitialPublishing == 2) {
    ...
    bError |= 0 == Publish(String(strTopic + "/$stats").c_str(), ipub_qos, true,
                           "uptime,signal,uptime-wifi,uptime-mqtt");
    bError |= 0 == Publish(String(strTopic + "/$stats/interval").c_str(),
                           ipub_qos, true, "60");
    ...
    bError |= 0 == Publish(String(strTopic + "/$nodes").c_str(), ipub_qos, true,
                           strNodes.c_str());
  ...
  if (iInitialPublishing == 3) {
    ...
    if (i < (int)vecNode.size()) {
      ...
      bError |= 0 == Publish(String(node.strTopic + "/$name").c_str(), ipub_qos,
                             true, node.strFriendlyName.c_str());
      bError |= 0 == Publish(String(node.strTopic + "/$type").c_str(), ipub_qos,
                             true, node.strType.c_str());
      ...
      if (bDebug)
        csprintf("NODE %i: %s has properties %s\n", i,
                 node.strFriendlyName.c_str(), strProperties.c_str());
#endif

      bError |= 0 == Publish(String(node.strTopic + "/$properties").c_str(),
                             ipub_qos, true, strProperties.c_str());
    ...
    if (i < (int)vecNode.size()) {
      ...
      if (j < (int)node.vecProperty.size()) {
        ...
        if (prop.bStandardMQTT) {
          ...
          bError |= 0 == Publish(String(prop.strTopic + "/$name").c_str(),
                                 ipub_qos, true, prop.strFriendlyName.c_str());
          bError |=
              0 == Publish(String(prop.strTopic + "/$settable").c_str(),
                           ipub_qos, true, prop.bSettable ? "true" : "false");
          bError |=
              0 == Publish(String(prop.strTopic + "/$retained").c_str(),
                           ipub_qos, true, prop.bRetained ? "true" : "false");
          bError |=
              0 == Publish(String(prop.strTopic + "/$datatype").c_str(),
                           ipub_qos, true, GetHomieDataTypeText(prop.datatype));
          if (prop.strUnit.length()) {
            bError |= 0 == Publish(String(prop.strTopic + "/$unit").c_str(),
                                   ipub_qos, true, prop.strUnit.c_str());
          }
          if (prop.strFormat.length()) {
            bError |= 0 == Publish(String(prop.strTopic + "/$format").c_str(),
                                   ipub_qos, true, prop.strFormat.c_str());
          }
...
uint16_t HomieDevice::PublishDirect(const String& topic, uint8_t qos,
                                    bool retain, const String& payload) {
...
uint16_t HomieDevice::Publish(const char* topic, uint8_t qos, bool retain,
  ...
  if (!ret) {  // failure
    if (!bSendError) {
      ...
      if ((int)(millis() - ulSendErrorTimestamp) >
          60000)  // a full minute with no successes
      {
        csprintf(
            "Full minute with no publish successes, disconnect and try "
            "again\n");
        mqtt.disconnect(true);
        bSendError = false;
        bConnecting = false;
...
void HSVtoRGB(float fHueIn, float fSatIn, float fBriteIn, float& fRedOut,
              float& fGreenOut, float& fBlueOut) {
...
unsigned long HomieDevice::GetReconnectInterval() {
  ...
  if (ulMqttReconnectCount >= 20)
    interval = 60000;
  else if (ulMqttReconnectCount >= 15)
    interval = 30000;
  else if (ulMqttReconnectCount >= 10)
    interval = 20000;
  else if (ulMqttReconnectCount >= 5)
    interval = 10000;
  return interval;
}
