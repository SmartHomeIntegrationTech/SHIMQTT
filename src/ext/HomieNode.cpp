...
const String& HomieProperty::GetValue() { return strValue; }
...
void HomieProperty::PublishDefault() {
  if (bSettable && bRetained && !bReceivedRetained && !bStandardMQTT) {
    ...
    if (strValue.length()) {
      ...
      csprintf(
          "%s didn't receive initial value for base topic %s so unsubscribe "
          "and publish default.\n",
...
bool HomieProperty::Publish() {
...
#ifdef HOMIELIB_VERBOSE
    csprintf("Empty value for %s encountered, substituting default. ",
             strID.c_str());
#endif
  }
  ...
  if (!pParent->pParent->mqtt.connected()) {
    ...
    csprintf("%s publishing \"%s\"\n", strFriendlyName.c_str(),
             strPublish.c_str());
#endif
    bRet = 0 != pParent->pParent->mqtt.publish(strTopic.c_str(), 2, bRetained,
                                               strPublish.c_str(),
                                               strPublish.length());
  }
  return bRet;
...
#ifdef HOMIELIB_VERBOSE
          csprintf(
              "%s ignoring invalid payload %s (float out of range "
              "%.04f:%.04f)\n",
              strFriendlyName.c_str(), strNewValue.c_str(), min, max);
#endif
          return false;
...
void HomieProperty::OnMqttMessage(char* topic, char* payload,
